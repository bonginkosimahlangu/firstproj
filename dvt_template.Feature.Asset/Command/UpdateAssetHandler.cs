...
{
    public class UpdateAssetHandler : CommandHandlerBase, IRequestHandler<UpdateAssetCommand, AssetViewModel>
    {
        public System.Threading.Tasks.Task<AssetViewModel> Handle(UpdateAssetCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(validateModel))
                {
                    ...
                    var Asset = commandService.UpdateAsset(request.Asset);
