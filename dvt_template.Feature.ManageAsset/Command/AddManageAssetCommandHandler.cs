...
{
    public class AddManageAssetCommandHandler : CommandHandlerBase, IRequestHandler<AddManageAssetCommand, ManageAssetViewModel>
    {
        public Task<ManageAssetViewModel> Handle(AddManageAssetCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(validateModel))
                {
                    ...
                    var manageAsset = commandService.NewManageAsset(request.ManageAsset);
